...
router.post('/create', async (req, res) => {
    const { f_name, l_name, mobile_no, address } = req.body;
    const conn = await connection(dbConfig).catch(e => { });
    const result = await create(
        conn,
        'customer',
        ['f_name', 'l_name', 'mobile_no', 'address'],
        [f_name, l_name, mobile_no, address]
    ).catch(e => {
...
router.get('/list', async (req, res) => {
    ...
    const list = await query(conn, listQuery({ limit, offset }, 'customer', 'is_active', is_active))
    const totalCount = await query(conn, totalCountQuery('customer'))
    res
        .status(201)
        .json(success("OK", {
            data: [...list],
